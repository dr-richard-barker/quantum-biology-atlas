...
def main() -> int:
    ...
    mannino_path = ROOT / "results" / "papers" / "Mannino2026" / "record.json"
    if mannino_path.exists():
        mannino = json.loads(mannino_path.read_text())
        m["ManninoLoci"] = mannino["loci_in_table"]
        m["ManninoCells"] = f"{mannino['cells_parsed']:,}"
        m["ManninoQbo"] = len(mannino["qbo_loci_covered"])
